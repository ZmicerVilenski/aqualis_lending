...
const func = async function ({ getNamedAccounts, deployments, ...hre }) {
    ...
    const AqualisOracle = (await (0, tx_1.getContract)("AqualisOracle", await addressesProviderInstance.getPriceOracle()));
    const configFallbackOracle = (await deployments.get(deploy_ids_1.FALLBACK_ORACLE_ID))
        .address;
    const stateFallbackOracle = await AqualisOracle.getFallbackOracle();
    if ((0, address_1.getAddress)(configFallbackOracle) === (0, address_1.getAddress)(stateFallbackOracle)) {
        console.log("[aqualis-oracle] Fallback oracle already set. Skipping tx.");
    }
    else {
        await (0, tx_1.waitForTx)(await AqualisOracle.setFallbackOracle(configFallbackOracle));
        console.log(`[Deployment] Added Fallback oracle ${configPriceOracle} to AqualisOracle`);
    }
...
func.id = `InitOracles:${env_1.MARKET_NAME}:aqualis-v3-core@${constants_1.V3_CORE_VERSION}`;
